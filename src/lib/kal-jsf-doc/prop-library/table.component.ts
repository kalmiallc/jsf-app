...
@Component({
  ...
  template       : `
      <div class="jsf-prop jsf-prop-table jsf-animatable" [ngClass]="htmlClass">
...
export class PropTableComponent extends AbstractPropLayoutComponent<JsfPropBuilderArray> implements OnInit {
  ...
  ngOnInit() {
    ...
    for (const header of this.layoutBuilder.headers) {
      header.decreaseLayoutLoadingCount();
    }
    for (const items of this.layoutBuilder.items) {
