...
import {
  ...
  JSF_RUNTIME_CONTEXT,
  JSF_THEME_GLOBAL_MODULE_FACTORY,
  JsfAppConfig,
  PreloadedModule
}                                         from '../common';
...
export class KalJsfDocComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  /**
   * Flag indicating whether the cdk overlay theme render should be forced on. Used for proxy doc component.
   */
  @Input()
  public forceRenderCdkOverlay = false;
  ...
  /**
   * Inject theme module factory, this is useful when you already have loaded theme module.
   */
  @Input() themeModuleFactory?: NgModuleFactory<any>;
  ...
  constructor(private loader: NgModuleFactoryLoader,
              ...
              @Optional() @Inject(JSF_THEME_GLOBAL_MODULE_FACTORY) public themeGlobalModuleFactory: any,
  ...
  ngOnInit() {
    ...
    // TODO use script injector service
    ...
    this.themeModuleFactory = this.themeModuleFactory || this.themeGlobalModuleFactory;
  }
  ...
  private preloadModules(prop: JsfProp): Promise<PreloadedModule[]> {
    let handlerPaths                          = this.getHandlerPaths(prop)
      .filter(x => ['any', 'component'].indexOf(x) === -1);
    const modules: Promise<PreloadedModule>[] = [];

    handlerPaths = uniq(handlerPaths);
    for (const path of handlerPaths) {
      modules.push(this.moduleLoaderService.loadHandlerModule(path));
    }
  ...
  private async buildDocumentWithModules(doc: JsfDocument, modules: PreloadedModule[]) {
    ...
    this.builder = await JsfBuilder.create(doc, {
      ...
      jsfComponentBuilder : this.jsfComponentBuilder,
      ...
      getPersistedValue: this.getPersistedValue.bind(this),
      setPersistedValue: this.setPersistedValue.bind(this),
    ...
    // CON-741: Update the linked builder instance here, because it may have been changed while the JsfBuilder.create
    // was being awaited.
  ...
  public getPersistedValue(persistType: 'localStorage' | 'query', key: string) {
    switch (persistType) {
      case 'query':
        return getUrlQuery()[key] || void 0;
      case 'localStorage':
        return localStorage.getItem(key);
    }
  }

  public setPersistedValue(persistType: 'localStorage' | 'query', key: string, value: any) {
    switch (persistType) {
      case 'query':
    ...
    try {
      ...
      if (this.enableThemeRender) {
        ...
        componentInstance = await this.themeRendererService.renderTheme(
          this.renderer,
          container,
          this.themeModuleFactory ? { moduleFactory: this.themeModuleFactory } : this.builder.doc.$theme,
          JsfThemeRenderMode.Form,
          {
            disableWrapperStyles : this.disableWrapperStyles,
            forceRenderCdkOverlay: this.forceRenderCdkOverlay
          });
      } else {
