...
export abstract class Node<T extends PIXI.Container> {
  ...
  public async render() {
    ...
    if (this._displayObject.interactive) {
  ...
  private subscribeToMouseDownEvent() {
    if (this.layoutData.events) {
      if (this.layoutData.events[NodeEvent.MouseDown]) {
        this._displayObject
        .on('pointerdown', (event: PIXI.InteractionEvent) => {
          const cursorGlobal = event.data.global;
          const spriteGlobal = this._displayObject.getGlobalPosition();
          this.clickOffset = new PIXI.Point(cursorGlobal.x - spriteGlobal.x, cursorGlobal.y - spriteGlobal.y);
          this.dragging = true;
          this.dispatchEvent(NodeEvent.MouseDown, { $positionX: event.data.getLocalPosition(this._displayObject.parent).x - this.clickOffset.x, $positionY: event.data.getLocalPosition(this._displayObject.parent).y - this.clickOffset.y });
          this.frameRate = this.renderer.ticker.frameRate;
          this.renderer.ticker.frameRate = 60;
        });
      }
    }
  }

  private subscribeToMouseUpEvent() {
    if (this.layoutData.events) {
      if (this.layoutData.events[NodeEvent.MouseUp]) {
        this._displayObject
        .on('pointerup', (event: PIXI.InteractionEvent) => {
          this.dragging = false;
          this.dispatchEvent(NodeEvent.MouseUp, { $positionX: event.data.getLocalPosition(this._displayObject.parent).x - this.clickOffset.x, $positionY: event.data.getLocalPosition(this._displayObject.parent).y - this.clickOffset.y });
          this.renderer.ticker.frameRate = this.frameRate;
        })
  ...
  private subscribeToDragEvent() {
    if (this.layoutData.events) {
      if (this.layoutData.events[NodeEvent.Drag]) {
        this._displayObject
  ...
  // // drag & drop
  // private onDragStart() {
  //   this.dispatchEvent(NodeEvent.MouseDown);
  // }

  // private onDragEnd() {
  //   this.dispatchEvent(NodeEvent.MouseUp);
  // }

  // private onDragMove() {
  //   this.dispatchEvent(NodeEvent.Drag);
  //   this.dispatchEvent(NodeEvent.Drag);
  // }
