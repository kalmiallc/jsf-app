...
@Component({
  ...
  template       : `
      <a *ngIf="href" class="jsf-animatable" [ngClass]="htmlClass" (click)="handleLayoutClick($event);" href="{{ href }}" target="_blank">{{ title }}</a>
      <a *ngIf="!href" class="jsf-animatable" [ngClass]="htmlClass" (click)="handleLayoutClick($event);" href="javascript:void(0)">{{ title }}</a>
  `,
