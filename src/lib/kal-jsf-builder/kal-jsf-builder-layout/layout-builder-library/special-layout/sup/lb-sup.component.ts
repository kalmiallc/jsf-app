...
@Component({
  selector       : 'jsf-lb-sup',
  template       : `
      <sub class="jsf-lb-sup" [ngClass]="getLayoutEditorClass()">{{ title }}</sub>
...
export class LbSupComponent extends AbstractLayoutBuilderComponent {

  @Input()
  layoutEditor: JsfLayoutEditor;

  get title(): string {
    return this.layout.title || 'Superscript';
  }
