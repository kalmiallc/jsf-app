...
@Component({
  selector       : 'jsf-filter-generic-select',
  template       : `
    <div class="handler jsf-filter-generic-select">
...
export class FilterGenericSelectComponent extends AbstractFilterPropComponent<JsfPropBuilderString> implements OnInit {
  ...
  get items(): { value: string, label: string }[] {
    return (this.filterItem.jsfProp.prop.handler as any).filterValues || (this.filterItem.jsfProp.prop.handler as any).values;
  }
