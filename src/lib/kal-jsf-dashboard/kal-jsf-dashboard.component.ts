...
import { debounce, filter, takeUntil, throttleTime } from 'rxjs/operators';
...
export class KalJsfDashboardComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  widgetInstances: {
    [key: string]: {
      ...
      filterGroupsInfo?: {
        [indexKey: string]: { hash: string, sharedHash: string }
      },
      ...
      if (this.widgetInstances[widgetKey].jsfBuilder
        ...
        this.widgetInstances[widgetKey].filterSubscription =
          (this.widgetInstances[widgetKey].jsfBuilder.propBuilder as JsfPropBuilderObject)
            ...
            // TODO statusChange should be replaced with valueChange
            .statusChange.pipe(filter((x => x !== PropStatus.Pending)))
            // .valueChange
  ...
  private onFormEventHook(data: JsfFormEventInterface): Promise<any> {
    if (data.event === 'data-source:reload') {
      const onlyGroupKey = data.value && data.value.onlyGroupKey;
      for (const widgetKey of Object.keys(this.widgetInstances)) {
        if (this.widgetInstances[widgetKey].filterGroupsInfo) {
          for (const groupKey of Object.keys(this.widgetInstances[widgetKey].filterGroupsInfo)) {
            if (onlyGroupKey) {
              if (onlyGroupKey === groupKey) {
                this.widgetInstances[widgetKey].filterGroupsInfo[groupKey].hash = undefined;
              }
            } else {
              this.widgetInstances[widgetKey].filterGroupsInfo[groupKey].hash = undefined;
            }
      ...
      return Promise.all(this.requestDataSources());
    }
  }

  private sendEventToWidgets(eventKey: string, eventData: any) {
    for (const widgetKey of Object.keys(this.dashboard.widgets)) {
      if (this.dashboard.widgets[widgetKey].dataSource) {
        ...
        this.widgetInstances[widgetKey].jsfBuilder.onExternalEvent(eventKey, eventData);
      }
    }
  }

  private registerActiveRequest(req: any) {
    this.activeRequests.push(req);
    this.sendEventToWidgets('data-source:stats:active', { count: this.activeRequests.length });
  }

  private unregisterActiveRequest(req: any) {
  ...
  private requestDataSources(): Promise<any>[] {
    if (this.edit) {
      return;
    }
    const dataSources: { [dataSource: string]: any[] }                                                                              = {};
    // const dataSourceSubscribers: { [dataSource: string]: true }                      = {};
    const dataSourcesGroups: { [dataSource: string]: { [groupKey: string]: { widgetKey: string, filters: any[], same: boolean } } } = {};
    ...
    for (const widgetKey of Object.keys(this.dashboard.widgets)) {
      ...
      if (this.dashboard.widgets[widgetKey].filter) {
        // if (!this.dashboard.widgets[widgetKey].dataSourceOnlyGroups && this.dashboard.widgets[widgetKey].dataSource)
        // { dataSourceSubscribers[this.dashboard.widgets[widgetKey].dataSource] = true; }
          ...
          const groupFilters = (rootProp.properties['filter'] as JsfPropBuilderArray)
            ...
            .forEach((x, i) => {
              ...
              if (this.widgetInstances[widgetKey].filterGroupsInfo[i]
                ...
                same = true; // skip filter group did not change
              } else {
                this.widgetInstances[widgetKey].filterGroupsInfo[i] = { hash: x.hash, sharedHash: null };
              }
              ...
              dataSourcesGroups[this.dashboard.widgets[widgetKey].filter][i].filters.push(x.value);
            })
        } else {
    ...
    const dataSourcesList: { dataSourceKey: string, groupKey: string, filter: any[] }[] =
            Object.keys(dataSources)
              // .filter(x => dataSourceSubscribers[x])
              .map(x => ({
                dataSourceKey: x,
    ...
    for (const sourceKey of Object.keys(dataSourcesGroups)) {
      const newSharedHash = hash.MD5(dataSources[sourceKey]);
      for (const groupKey of Object.keys(dataSourcesGroups[sourceKey])) {
        const sharedHash = this.widgetInstances[dataSourcesGroups[sourceKey][groupKey].widgetKey].filterGroupsInfo[groupKey].sharedHash;
        if (dataSourcesGroups[sourceKey][groupKey].same) {
          if (sharedHash === newSharedHash) {
            continue;
          } else {
            this.widgetInstances[dataSourcesGroups[sourceKey][groupKey].widgetKey].filterGroupsInfo[groupKey].sharedHash = newSharedHash;
          }
        }
        ...
        dataSourcesList.push({
          ...
          filter       : [
            ...
            ...dataSourcesGroups[sourceKey][groupKey].filters
          ],
          groupKey     : groupKey
        });
      }
    ...
    if (this.dataSourceRequest && dataSourcesList.length) {
      for (const dataSource of dataSourcesList) {
        ...
        if (request$) {
          this.registerActiveRequest(request$);
          requests.push(request$.pipe(takeUntil(this.ngUnsubscribe)).toPromise());
          request$
            .pipe(takeUntil(this.ngUnsubscribe))
            .subscribe(x => {
              this.removeDataSourceRequestFromActiveList(req);
              this.unregisterActiveRequest(request$);
