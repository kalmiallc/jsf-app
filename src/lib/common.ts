...
export interface JsfAppConfig {
  ...
  /**
   * Address to redirect to when user clicks on powered-by layout.
   */
  poweredByUrl: string;

  syncLayoutRendering?: boolean;

  builder?: {
    assetsBasePath?: string;

    disablePreviewThemeRendering?: boolean;
    disableProxyPreviewRendering?: boolean;
  };
}
...
/**
 * Provide this token with a list of all available handlers.
 */
export const JSF_HANDLERS = new InjectionToken<string[]>('JSF_HANDLERS');
 ...
 * ]
 */
export const JSF_API_SERVICE = new InjectionToken<any>('JSF_API_SERVICE'); // TODO type
...
export const JSF_THEME_GLOBAL_MODULE_FACTORY        = new InjectionToken<JsfRuntimeContext>('JSF_THEME_GLOBAL_MODULE_FACTORY');
